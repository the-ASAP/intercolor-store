export function initAllCategories(apiUrl, endpoint, root = document) {
  ...
  searchIn.addEventListener("input", () => {
    ...
    searchTimer = setTimeout(() => {
      if (!q) {
        // if empty, fall back to full catalog
        loadSections();
      } else {
        loadFiltered(q);
      }
  ...
  function loadSections() {
    fetch(`${apiUrl}/${endpoint}`)
      ...
      .then(({ sections }) => {
        ...
        // initial pick
        const firstCat = pickFirstCat(sections);
        if (firstCat) renderSubcats(firstCat);
      })
      .catch((err) => console.error(err));
  }

  function loadFiltered(param) {
    fetch(`${apiUrl}/v1/catalog/section/${encodeURIComponent(param)}/filter/`)
      .then((r) => r.json())
      .then(({ sections }) => {
        renderCategories(sections);
        // initial pick
  ...
  function pickFirstCat(sections) {
    const top = Object.values(sections).find((s) => s.depth_level === "1");
    return top && top.sections && top.sections[0];
  }

  // ─── Render depth-3 subcategories (for “sections” mode) ──────────────────
